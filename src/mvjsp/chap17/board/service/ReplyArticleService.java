...
import mvjsp.chap17.board.dao.*;
import mvjsp.chap17.board.model.*;
import mvjsp.jdbc.*;
...
public class ReplyArticleService {
	...
	LastChildAlreadyExistException{
		...
		try {
			...
			try {
				...
				else throw (LastChildAlreadyExistException)e;
			}
			String searchMaxSeqNum = parent.getSequenceNumber();
			String searchMinSeqNum = getSearchMinSeqNum(parent);
			
			String lastChildeSeq = articleDao.selectLastSequenceNumber(conn, 
			...
			if(articleId == -1) {
				throw new RuntimeException("DB삽입 실패:"+articleId);
			}
			conn.commit();
			...
			JdbcUtil.rollback(conn);
			throw new SQLException("DB 에러."+e.getMessage(),e);
		}finally {
			if(conn!= null) {
				try {
	...
	private String getSequenceNumber(Article parent, String lastChildSeq)
			throws LastChildAlreadyExistException {
		...
		if (lastChildSeq == null) { 
			...
			if (orderOfLastChildSeq.equals("00")) {
				throw new LastChildAlreadyExistException(
						"마지막 자식글이 이미존재합니다 :" + lastChildSeq);
			}
